...
void Button::OnLongPress(std::function<void()> callback, uint16_t ms) {
    if (button_handle_ == nullptr) {
        return;
    }
    ...
    button_event_args_t args = {
        .long_press = { .press_time = ms }
    };

    iot_button_register_cb(button_handle_, BUTTON_LONG_PRESS_START, (ms)? &args : nullptr, [](void* handle, void* usr_data) {
        Button* button = static_cast<Button*>(usr_data);
